...
async function writeBuildMeta(targetDir) {
  ...
  let lucyPrompt = "V9.81";
  try {
    const releaseSrc = await readFile(
      path.join(artifactDir, "src", "lib", "lucyRelease.ts"),
      "utf8"
    );
    const m = releaseSrc.match(/LUCY_PROMPT_VERSION\s*=\s*"([^"]+)"/);
    if (m?.[1]) lucyPrompt = m[1];
  } catch {
    /* keep default */
  }
  const meta = {
...
globalThis.__dirname = __bannerPath.dirname(globalThis.__filename);
  ...
  const reparacionesSrc = path.resolve(artifactDir, "public/reparaciones");
  await cp(reparacionesSrc, path.join(distDir, "reparaciones"), { recursive: true });
  console.log("[build] Reparaciones copiado a dist/reparaciones/");
