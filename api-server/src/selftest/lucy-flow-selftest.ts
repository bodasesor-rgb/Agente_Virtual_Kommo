...
¿Cuál nivel prefieres? El detalle exacto de inclusiones te lo confirma el equipo en la cotización.`;
  ...
  // ─── V9.69 — A15566 Lynn: rangos de horario / a partir de / waiver ───
  await test("156. V9.69 — A15566 Lynn: de 3pm a 11pm, a partir de, sin horario aún", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.69");
  ...
  // ─── V9.69 — A15581 Mariana: horario en palabras / dos propuestas / DJ ───
  await test("157. V9.69 — A15581 Mariana: cuatro, dual propuesta, DJ inclusión/acotado", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.69");
  ...
  // ─── V9.69 — A15620 Mara: promo pedido mínimo ≠ invitados; hora envío ≠ horario ───
  await test("158. V9.69 — A15620 Mara: promo CierreRapido no captura 35 pax ni hora de envío", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.69");
    const maraPromo = [
      "Hola, escribo por la promo de cierre rápido (10% de descuento).",
      "Código: CierreRapido",
  ...
  // ─── V9.69 — A15701 Alejandra: Puerto Vallarta = ciudad, no bucle ───
  await test("159. V9.69 — A15701 Alejandra: Puerto Vallarta no repregunta ciudad", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.69");
  ...
  // ─── V9.69 — A15707 danymelgozza: primer mensaje ≠ "ya platicamos" ───
  await test("160. V9.69 — A15707 danymelgozza: cotización inicial no es envío de cotización", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.69");
    const opening =
  ...
  // ─── V9.69 — A15708 Itzel: no repetir presentación Lucy tras nombre ───
  await test("161. V9.69 — A15708 Itzel: una sola presentación Lucy", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.69");
    const firstIntro =
  ...
  // ─── V9.69 — A15705 Karla: "Sería De Catering" ≠ nombre ───
  await test("162. V9.69 — A15705 Karla: Sería De Catering no es nombre", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.69");
