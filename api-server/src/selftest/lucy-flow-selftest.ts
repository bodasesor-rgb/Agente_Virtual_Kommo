...
¿Cuál nivel prefieres? El detalle exacto de inclusiones te lo confirma el equipo en la cotización.`;
  ...
  await test("133. V9.35 — banquete Torreón primer turno pide fecha/invitados", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.61");
    const filled = new Set([
      "Nombre del cliente",
      "Tipo de evento",
      "Requerimientos o servicios",
  ...
  // ─── V9.61 — A15566 Lynn: rangos de horario / a partir de / waiver ───
  await test("156. V9.61 — A15566 Lynn: de 3pm a 11pm, a partir de, sin horario aún", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.61");
  ...
  // ─── V9.61 — A15581 Mariana: horario en palabras / dos propuestas / DJ ───
  await test("157. V9.61 — A15581 Mariana: cuatro, dual propuesta, DJ inclusión/acotado", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.61");
  ...
  // ─── V9.61 — A15620 Mara: promo pedido mínimo ≠ invitados; hora envío ≠ horario ───
  await test("158. V9.61 — A15620 Mara: promo CierreRapido no captura 35 pax ni hora de envío", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V9.61");
    const maraPromo = [
      "Hola, escribo por la promo de cierre rápido (10% de descuento).",
      "Código: CierreRapido",
