...
¿Cuál nivel prefieres? El detalle exacto de inclusiones te lo confirma el equipo en la cotización.`;
  ...
  await test("121. V8.93 — voz humana preferida + cierre sin upsell + prompt", () => {
    assert.ok(/^V(8\.9[3456789]|9\.\d{2}|10\.\d{2})$/.test(LUCY_PROMPT_VERSION), LUCY_PROMPT_VERSION);
  ...
  await test("122. V8.94 — Gemini Flash-Lite provider + conversión mensajes", () => {
    assert.ok(/^V(9\.\d{2}|10\.\d{2})$/.test(LUCY_PROMPT_VERSION), LUCY_PROMPT_VERSION);
  ...
  await test("127. A14936 — Lety Hacienda aliados → proveedor, no formulario evento", async () => {
    ...
    // A16075: sin cuestionario completo aún pide datos (no cierra en silencio).
    assert.ok(/ofrecen|datos r[aá]pidos|cat[aá]logo/i.test(reply), reply);
    ...
    assert.ok(healthSrc.includes("proveedor-questionnaire-sheets"));
  });
  ...
  // ─── A15956 Verónica — entelado ≠ mobiliario / mesas / periqueras ───
  await test("A15956 — entelado para techo no cae en menú de mobiliario", () => {
    for (const msg of [
      "entelado",
      "entelados",
      "entelado para techo",
      ...
      assert.equal(shouldOfferOptionsBeforeDetail({
        currentMessage: msg,
        history: [],
      }), null, msg);
    }
    ...
    assert.equal(
      resolveCatalogWebSlug("entelado para techo"),
      "entelados-para-techo"
    );
    assert.ok(
      /entelados-para-techo/i.test(getCatalogWebUrlForQuery("entelado") ?? ""),
    ...
    assert.ok(/entelados-para-techo|entelado/i.test(early), early.slice(0, 500));
    ...
    assert.ok(!/mesas y sillas.*periqueras|periqueras.*ambas/i.test(afterMedidas), afterMedidas.slice(0, 500));
  });
  ...
  await test("146. A15308 — solo nombre: Mucho gusto + embudo, sin felicitar", () => {
    ...
    assert.equal((live.match(/Soy Lucy/gi) ?? []).length, 0, live.slice(0, 300));
  ...
  await test("128. V9.28 — solo vs completo en todas las ramas completas", () => {
    assert.ok(/^V(9\.(2[89]|[3-9]\d)|10\.\d{2})$/.test(LUCY_PROMPT_VERSION), LUCY_PROMPT_VERSION);
    const csv = [
      '"Servicio","Nivel","Precio Unitario","Precio Minimo de salida","Catálogo Revisado","Que Incluye","Link catalogo"',
  ...
  await test("133. V9.35 — banquete Torreón primer turno pide fecha/invitados", () => {
    assert.equal(LUCY_PROMPT_VERSION, "V10.06");
    const filled = new Set([
      "Nombre del cliente",
      "Tipo de evento",
      "Requerimientos o servicios",
  ...
  await test("138. V9.41 — A15383 Kelia: ciudad, banquetes, LED≠luz, no spam (todas las ramas)", () => {
    ...
    assert.ok(parseServicesFromText(banquetQ).includes("Banquete"));
  ...
  await test("147. V9.49 — A15503 Good: loza y plato postre = vajilla, no postres", () => {
    ...
    assert.ok(isTablewareRequestText(brief));
    const services = parseServicesFromText(brief);
    assert.ok(services.includes("Vajillas"), services.join("|"));
    assert.ok(!services.includes("Mesa de postres"), services.join("|"));

    const web = parseWebLeadBrief(brief);
    assert.ok(web?.requerimientos_evento?.includes("Vajillas"), web?.requerimientos_evento);
    assert.ok(!/mesa de postres/i.test(web?.requerimientos_evento ?? ""), web?.requerimientos_evento);

    const declineFamilies = clientDeclinesServiceFamilies("No necesito mesa de postre");
    assert.ok(declineFamilies.includes("dulces"), declineFamilies.join(","));
    assert.ok(!declineFamilies.includes("mobiliario"), declineFamilies.join(","));

    const onlyVajilla = parseServicesFromText("Únicamente vajilla");
    assert.deepEqual(onlyVajilla, ["Vajillas"]);

    const declineGuard = runGuards({
      aiResponse: "Perfecto, anoto mesa de postres.",
      extracted: emptyExtracted({
        nombre: "Good",
        requerimientos_evento: "Mesa de postres, Vajillas",
        num_invitados: 50,
      }),
      filledSet: new Set(["Nombre del cliente", "Requerimientos o servicios", "Número de invitados"]),
      readyForClosing: false,
      currentMessage: "No necesito mesa de postre",
    });
  ...
  await test("149. V9.51 — A15508 Betiana: 40 invitadas y anti-repetición", () => {
    ...
    assert.equal(
      recoverInvitadosFromUserTexts(
        ["40 sillas, 40 invitadas"],
        "perdon ya me preguntaste eso",
        { mobiliarioRenta: true }
      ),
      40
    );
    assert.ok(clientComplainsAboutRepeat("perdon ya me preguntaste eso"));

    const extracted = emptyExtracted({
      nombre: "Betiana",
      tipo_evento: "baby shower",
      requerimientos_evento: "Mobiliario, Sillas Crossback",
    ...
    assert.equal(extracted.num_invitados, 40);
    assert.ok(filled.has("Número de invitados"));
    assert.notEqual(getNextPendingField(extracted, filled), "invitados");

    const guarded = runGuards({
      aiResponse: "Con gusto. ¿Tienen un estimado de invitados? Si aún no, un rango sirve.",
      extracted,
      filledSet: filled,
      readyForClosing: false,
  ...
  await test("153. V9.55 — A15539 Jorge: horario/carpa/DJ no/mobiliario/Atlixco/callback", () => {
    ...
    assert.ok(/cocktail|12|comida|2/i.test(parseHorarioFromText("cocktail a las 12:00\ncomida a las 2:00") ?? ""));
    ...
    assert.ok(parseServicesFromText("necesito bar tender, DJ no, capra sí").includes("Coctelería"));
    assert.ok(parseServicesFromText("necesito bar tender, DJ no, capra sí").includes("Carpas"));
    assert.ok(!parseServicesFromText("necesito bar tender, DJ no, capra sí").some((s) => /^DJ$/i.test(s)));
    assert.ok(clientDeclinesServiceFamilies("necesito bar tender, DJ no, capra sí").includes("entretenimiento"));

    assert.ok(/Atlixco/i.test(parseZonaFromText("atlixco") ?? ""));
    assert.ok(isVenueSpaceDetail("será en un jardin"));
    assert.ok(isVenueSpaceDetail("jardín"));
    ...
    assert.ok(isBodasesorCompanyPhone("+52 55 4008 0373"));
    const phoneCleared = applyCrmWriteInvariants(
      emptyExtracted({ telefono: "55 4008 0373" }),
      ["este es mi tel"]
    );
    assert.equal(phoneCleared.extracted.telefono, null);

    const mobiliarioInc = buildSpecificInclusionItemReply(
      "el servicio completo incluye mobiliario correcto?",
      "Paella"
    );
    assert.ok(mobiliarioInc && /mobiliario/i.test(mobiliarioInc) && /s[ií]|correcto|incluye/i.test(mobiliarioInc), mobiliarioInc ?? "");
    ...
    assert.ok(!/^\s*¿Qué tipo de evento es\?\s*$/i.test(carpaQ.trim()), carpaQ.slice(0, 300));
    ...
    assert.ok(/sin\s+DJ|no\s+incluimos\s+entretenimiento|Cocteler|bartender|carpa/i.test(djNo), djNo.slice(0, 400));
    assert.ok(
      /Cocteler/i.test(checklist.requerimientos_evento ?? "") ||
        /sin\s+DJ|Cocteler|carpa/i.test(djNo),
      checklist.requerimientos_evento
    );
    assert.ok(!/\bDJ\b/i.test(checklist.requerimientos_evento ?? "") || /sin\s+DJ/i.test(djNo));

    const mobiliarioQ = runGuards({
      aiResponse: "Perfecto, Jorge. Sí, contamos con *mobiliario*. ¿Qué es lo que buscas?",
      extracted: emptyExtracted({
        nombre: "Jorge",
        requerimientos_evento: "Paella",
      }),
      ...
      history: [{ role: "assistant", content: "¿Cuál te late más? Solo alimentos o servicio completo" }],
    });
    assert.ok(/incluye|correcto|s[ií]/i.test(mobiliarioQ) && /mobiliario/i.test(mobiliarioQ), mobiliarioQ.slice(0, 400));
    assert.ok(!/Qu[eé] es lo que buscas|Mesas\n|•\s*Sillas/i.test(mobiliarioQ), mobiliarioQ.slice(0, 400));

    const ciudad = runGuards({
      aiResponse: "Con gusto. Claro. Para *comida* del evento, ¿qué te gustaría? • Un *banquete*…",
      extracted: emptyExtracted({
        nombre: "Jorge",
        tipo_evento: "primera comunión",
      ...
      history: [
        {
          role: "assistant",
          content: "Listo, Jorge. Para cotizar bien necesito al menos la *ciudad* del evento. ¿En qué ciudad está?",
        },
      ],
    });
    ...
    const callback = runGuards({
      aiResponse: "Perfecto, Jorge. ¿Qué servicios te gustaría ir armando?",
      extracted: emptyExtracted({
        nombre: "Jorge",
  ...
  await test("156. V9.70 — A15566 Lynn: de 3pm a 11pm, a partir de, sin horario aún", () => {
    ...
    assert.ok(!/horario lo planean|Sigo aqu[ií]|confirmas ese dato/i.test(range), range.slice(0, 400));
    assert.ok(/3:00|11:00|anot/i.test(range), range.slice(0, 400));
    ...
    assert.ok(!/horario lo planean/i.test(defer), defer.slice(0, 400));
  });
  ...
  await test("157. V9.70 — A15581 Mariana: cuatro, dual propuesta, DJ inclusión/acotado", () => {
    ...
    const marianaBase = emptyExtracted({
    ...
    const horario = runGuards({
      aiResponse: "¿En qué horario lo planean?",
      extracted: marianaBase,
      filledSet: new Set(marianaFilled),
      readyForClosing: false,
      currentMessage: "Sería temprano, a partir de las cuatro",
      history: [{ role: "assistant", content: "¿A qué hora sería el evento?" }],
    });
    assert.ok(!/horario lo planean|a qu[eé] hora ser[ií]a/i.test(horario), horario.slice(0, 400));

    const djInfo = runGuards({
      aiResponse: "¡Claro! *DJ* la anoto para tu cotización.",
      extracted: { ...marianaBase, horario_evento: "a partir de las 4" },
      filledSet: new Set([...marianaFilled, "Horario del evento"]),
      readyForClosing: false,
      currentMessage: "Vi en su página que hay la opción de DJ ¿qué es lo que incluiría?",
    });
    assert.ok(/equipo|micr[oó]fono|iluminaci/i.test(djInfo), djInfo.slice(0, 400));
    assert.ok(!/la anoto para tu cotizaci/i.test(djInfo), djInfo.slice(0, 400));
  });
  ...
  await test("158. V9.70 — A15620 Mara: promo CierreRapido no captura 35 pax ni hora de envío", () => {
    ...
    assert.ok(isPromoTemplateMessage(maraPromo));
    assert.ok(isPromoMinimumGuestLine("Pedido mínimo: 35 personas."));
    assert.equal(parseInvitadosFromText(maraPromo), null);
    assert.equal(parseHorarioFromText(maraPromo), null);
    assert.ok(!/35\s*personas/i.test(buildRichBriefAcknowledgment(maraPromo)));
    ...
    const promoReply = runGuards({
      ...
      readyForClosing: false,
      currentMessage: maraPromo,
      forceFirstPresentation: true,
    });
    assert.equal(promoReply.match(/35\s*personas/gi)?.length ?? 0, 0, promoReply.slice(0, 400));
  });
  ...
  await test("159. V9.70 — A15701 Alejandra: Puerto Vallarta no repregunta ciudad", () => {
    ...
    const reply = runGuards({
      aiResponse: "Perfecto, Alejandra. Claro. Alejandra, ¿en qué ciudad sería?",
      extracted,
      filledSet: filled,
      readyForClosing: false,
      ...
      history: [
        {
          role: "assistant",
          content:
            "¿Podrías indicarme en qué lugar o alcaldía se llevará a cabo?",
        },
        { role: "user", content: "Puerto Vallarta" },
        { role: "assistant", content: "Listo. Alejandra, ¿me confirmas la *ciudad* del evento?" },
        { role: "user", content: "Puerto Vallarta" },
        { role: "assistant", content: "Perfecto, Alejandra. Claro. Alejandra, ¿en qué ciudad sería?" },
      ],
    });
    assert.ok(isFieldSatisfied("zona", filled, extracted));
  ...
  await test("162. V9.70 — A15705 Karla: Sería De Catering no es nombre", () => {
    ...
    const reply = runGuards({
      ...
      extracted: emptyExtracted({ nombre: "Karla Rodríguez" }),
      filledSet: new Set(["Nombre del cliente"]),
      readyForClosing: true,
  ...
  await test("163. V9.71 — A15775 Pamela: municipio GDL y meta-ciudad", () => {
    ...
    const reply = runGuards({
      aiResponse:
        "Perfecto, Pamela. Anoto la ubicación en *esa es la ciudad*. ¿A qué correo le paso la info?",
      extracted,
      filledSet: new Set([
        "Nombre del cliente",
        "Requerimientos o servicios",
      ...
      history: [
        ...
        {
          role: "assistant",
          content: "Perfecto, Pamela. ¿En qué ciudad sería?",
        },
      ],
      whatsappDisplayName: "Pamela Ale",
    });
    assert.match(reply, /tlaquepaque/i);
    assert.ok(!/\*esa es la ciudad\*/i.test(reply), reply.slice(0, 400));
