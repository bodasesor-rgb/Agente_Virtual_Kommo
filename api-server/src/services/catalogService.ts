 ...
 * V9.27: si hay Solo Alimentos + Basico/Tradicional/Premium, NO vuelques los 4:
 * primero solo alimentos vs servicio completo; si elige completo → teaser 3 niveles.
 */
function buildServiceNivelChoiceAnswer(
  ...
  // Multi-servicio hermanas: si el pedido es concreto, acotar a ese SKU.
  ...
  // V9.27: embudo solo alimentos vs servicio completo.
  if (serviceHasSoloVsCompleto(rowsForChoice)) {
    if (queryWantsSoloAlimentos(q)) {
      const soloRow = findNivelRow(rowsForChoice, /solo\s+alimentos/i);
      if (soloRow) return buildExactRowDetailAnswer(soloRow);
    }
    if (queryWantsCompletoMode(q) && !queryWantsSpecificCompletoNivel(q)) {
      return buildCompletoNivelesTeaser(svc, rowsForChoice);
    }
    if (!queryWantsSpecificCompletoNivel(q) && !queryWantsSoloAlimentos(q)) {
      return buildSoloVsCompletoModeAnswer(svc, rowsForChoice);
    }
    // Nivel concreto (Básico/Tradicional/Premium): cae al listado corto o detalle abajo.
  }
...
function isSoloAlimentosNivelLabel(n: string): boolean {
  return /solo\s+alimentos/i.test(n);
}

function isCompletoPackageNivelLabel(n: string): boolean {
  return /\b(b[aá]sic[oa]?|tradicional|premium)\b/i.test(n) && !isSoloAlimentosNivelLabel(n);
}

/** Sheet con Solo Alimentos + al menos 2 paquetes completos (Basico/Tradicional/Premium). */
export function serviceHasSoloVsCompleto(rows: SheetCatalogRow[]): boolean {
...
function findNivelRow(rows: SheetCatalogRow[], re: RegExp): SheetCatalogRow | undefined {
  return rows.find((r) => re.test(extractNivelLabel(r)));
}

function formatRowPriceShort(row: SheetCatalogRow | undefined): string {
...
function queryWantsCompletoMode(q: string): boolean {
  return /\bservicio\s+completo\b/i.test(q) || /(?:^|[^\p{L}])completo(?:[^\p{L}]|$)/iu.test(q);
}

function queryWantsSpecificCompletoNivel(q: string): boolean {
...
export function buildSoloVsCompletoModeAnswerWithPrices(
  ...
  const basicoRow =
    findNivelRow(rows, /\bb[aá]sic/i) ||
    rows
      .filter((r) => isCompletoPackageNivelLabel(extractNivelLabel(r)))
      .sort((a, b) => {
...
/** Tras elegir servicio completo: 3 niveles sin volcar precios/inclusiones. */
export function buildCompletoNivelesTeaser(
  svc: string,
  rows: SheetCatalogRow[]
): string {
  const completo = uniqueNiveles(rows).filter(isCompletoPackageNivelLabel);
  const pretty = completo
    .slice(0, 3)
    .map((n) => `*${n.replace(/^Basico$/i, "Básico")}*`)
    .join(", ");
...
export function messageOffersLevelsWithoutInclusions(text: string | null | undefined): boolean {
  ...
  const soloDump =
    /solo\s+alimentos/i.test(t) &&
    mentionsTriad &&
    (/1\.\s*\*?solo\s+alimentos/i.test(t) || /manejamos estos niveles/i.test(t));
  return (
    onlyTeamPlaceholder ||
    soloDump ||
...
export type SpecificInclusionItem =
  | "bebidas"
  | "meseros"
  | "vajilla"
  | "cristaleria"
...
const INCLUSION_ITEM_PATTERNS: Record<SpecificInclusionItem, RegExp> = {
  bebidas:
    /\bbebidas?\b|\brefrescos?\b|\bbarman\b|\bbarra\s+de\s+bebidas?\b|\bvitroleros?\b|\bagua\s+fresca\b/i,
...
function inclusionEvidenceRegex(
  item: SpecificInclusionItem,
  serviceLabel: string
): RegExp {
  if (item === "bebidas") {
    // Coffee Break: el café/té ES la bebida. Otros alimentos: no usar "café" suelto
    // (el PDF de otro nivel contamina el slice).
...
/** Ventana PDF acotada al nivel (evita arrastrar la barra del nivel siguiente). */
function pdfWindowForNivel(service: string, nivel: string): string {
  const raw =
    buildPdfInclusionReply(`${service} ${nivel}`) ||
  ...
  const nivelRe = nivel
    .replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
    .replace(/\s+/g, "\\s+");
  const start = body.search(new RegExp(nivelRe, "i"));
  if (start < 0) return body.slice(0, 420);
 ...
 * (incluye typos: Inclue, yncluye). Todas las ramas/servicios.
 */
export function clientAsksSpecificInclusionItem(
  ...
  if (
    !/\b(incluye|incluyen|incluir|trae|traen|lleva|llevan|viene|vienen|tiene|tienen|con\s+bebidas?|sin\s+bebidas?)\b/i.test(
      t
    ) &&
    !/\bsi\b.{0,50}\bincluye/i.test(t) &&
    !/\bviene(n)?\s+con\b/i.test(t)
  ) {
    return null;
  }
...
function resolveInclusionServiceLabel(
  query: string,
  serviceHint?: string | null
): string | null {
  ...
  if (serviceHint?.trim()) {
    const first = serviceHint
      .split(/,| y /i)
      .map((s) => s.trim())
      .find((s) => parsePrimaryService(s) || /\b(canap|bocadillo|banquete|taquiza|coffee|barra|mesa|sushi|pizza|paella|desayuno|brunch)\b/i.test(s));
...
function formatNivelPrice(row: SheetCatalogRow): string {
  if (!row.tienePrecio || !row.precio) return "";
  const unit = row.unidad?.trim() ? ` ${row.unidad.trim()}` : "";
  return ` (${row.precio}${unit})`;
}
...
/**
 * Respuesta a "¿incluye X?" para CUALQUIER servicio del catálogo.
 * Cruza niveles del Sheet + texto PDF (Aprendizaje) y resume sí/no por nivel.
 */
export function buildSpecificInclusionItemReply(
  ...
  const rows =
    resolved && resolved.kind !== "category"
      ? [...new Map(resolved.rows.map((r) => [extractNivelLabel(r), r])).values()]
      : [];
  ...
  for (const row of rows.slice(0, 8)) {
    ...
    const has = hasSheet || hasPdf;
    // Solo Alimentos = comida; bebidas/alcohol NO, salvo que el Sheet lo diga explícito.
    const foodOnlyNoDrinks =
      (item === "bebidas" || item === "alcohol") &&
      soloAlimentos &&
      !hasSheet &&
      !/\b(bebidas?|refrescos?|vitroleros|barra\s*:|alcohol)\b/i.test(sheetInc);
    const negated =
      foodOnlyNoDrinks ||
      new RegExp(
    ...
    else if (has && !negated) yes.push(bullet);
    else if (negated) no.push(bullet);
    else unclear.push(bullet);
  }

  // Add-on típico: "Agrega Barra de Bebidas — $180"
  let addonLine: string | null = null;
  if (item === "bebidas" || item === "alcohol") {
    const addonPdf =
      buildPdfInclusionReply(`${service} Barra de Bebidas`) ||
      buildPdfInclusionReply(`${service} agrega barra de bebidas`) ||
      buildPdfInclusionReply(`Agrega Barra de Bebidas ${service}`);
    ...
    if (/agrega\s+barra\s+de\s+bebidas|barra\s+de\s+bebidas\s*[—\-–]/i.test(addonBlob) || priceMatch) {
      const p = priceMatch?.[1] ? ` (+$${priceMatch[1]} /pp)` : "";
      addonLine = `• Se puede *agregar Barra de Bebidas*${p} si quieren barra más completa.`;
    }
  }

  // Si no hay filas Sheet, caer al PDF del servicio.
  if (!rows.length) {
    const fromPdf =
      buildPdfInclusionReply(`${service} ${query}`) ||
      buildPdfInclusionReply(`${service} ${item}`) ||
      buildPdfInclusionReply(service);
    if (!fromPdf) {
      const web = getCatalogWebUrlForQuery(service);
      return ensureCatalogWebLink(
        `Para *${service}*, el detalle de *${item}* lo confirmo con el catálogo${web ? "" : ""}. Nuestro equipo te precisa exactamente qué incluye cada nivel.`,
        service
      );
    }
    return ensureCatalogWebLink(
  ...
  const lines: string[] = [`Sobre *${item}* en *${service}*:`, ""];
  if (yes.length) {
    lines.push(`*Sí incluye* en:`);
    lines.push(...yes);
    lines.push("");
  }
  if (no.length) {
  ...
  if (!yes.length && !no.length) {
    // Sin evidencia clara: pegar el mejor bloque PDF + precios Sheet.
    ...
    return ensureCatalogWebLink(
      `Sobre *${item}* en *${service}*:\n\n${body}\n\n¿De qué nivel te paso el detalle completo?`,
      service
    );
  }
  if (addonLine) {
  ...
  if (unclear.length && yes.length + no.length < rows.length) {
    // omitir unclear ruidoso si ya hay sí/no suficientes
  }

  lines.push("¿Te late alguno de estos niveles o te detallo uno en concreto?");
  return ensureCatalogWebLink(lines.filter((l, i, a) => !(l === "" && a[i - 1] === "")).join("\n"), service);
}
...
export function buildCatalogPriceAnswer(query: string): string | null {
  ...
  if (resolved.kind === "service") {
    ...
    if (priced.length > 1) {
      ...
      // V9.27: precio → solo vs completo (no 4 bullets).
      if (
        serviceHasSoloVsCompleto(unique) &&
        !queryWantsSpecificCompletoNivel(query) &&
        !queryWantsSoloAlimentos(query)
      ) {
        if (queryWantsCompletoMode(query)) {
  ...
  const baseName = resolved.serviceName ?? unique[0]!.servicio;
  // V9.27: no volcar 4 precios si aplica solo vs completo.
...
export function buildCatalogServiceDetailAnswer(query: string): string | null {
  ...
  if (resolved?.kind === "service") {
    // V9.27: "Taquiza Servicio completo" → teaser 3 niveles (no dump).
    if (
      serviceHasSoloVsCompleto(resolved.rows) &&
      queryWantsCompletoMode(query) &&
      !queryWantsSpecificCompletoNivel(query)
    ) {
