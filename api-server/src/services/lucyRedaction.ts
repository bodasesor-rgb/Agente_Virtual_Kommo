...
export function buildRedactionBriefing(input: RedactionBriefingInput): string {
  ...
  lines.push(
    ...
    `Si preguntan qué incluye un servicio/nivel: SOLO texto del campo Incluye del catálogo. Si está vacío → manda el link del catálogo web del servicio (bodasesor.com/catalogos/…) o el hub general; no digas solo "el equipo lo confirma" sin link. Jamás rellenes con cervezas, vinos, platillos ni marcas inventadas.`,
    SERVICE_KNOWLEDGE_GOLDEN_RULE,
