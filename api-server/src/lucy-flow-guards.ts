...
export function syncFilledFromExtracted(filledSet: Set<string>, extracted: ExtractedData): void {
  ...
  // Compat: tests/CRM viejos con etiqueta combinada.
  ...
  // A15443 / A15419: horario suelto / discurso no cierran Fecha; van a Horario.
  hydrateScheduleFields(extracted);
...
export function syncRichBriefIntoExtracted(
  ...
  if (!extracted.horario_evento?.trim()) {
    const h = parseHorarioFromText(text);
...
const FIELD_ASK_PATTERNS: Record<PendingField, RegExp> = {
  ...
  fecha:
    /(?:^|[\s,.])fecha(?:[\s,.]|$)|cu[aá]ndo\s+(ser[ií]a|es|tienen|tengan)|d[ií]a\s+(del\s+evento|tienen|tengan|definido)|agenda|definiendo|definido|definir|siguen\s+viendo|opciones\s+de\s+fecha|para\s+qu[eé]\s+d[ií]a|qu[eé]\s+d[ií]a|siguen\s+sin\s+fecha|ya\s+tienen\s+fecha/i,
  horario:
...
export function getNextPendingField(
  ...
  const hasFecha =
    filled.has(CRM_FECHA_LABEL) || isUsableFechaEvento(extracted.fecha_evento);
  if (!hasFecha) return "fecha";

  const hasHorario =
    filled.has(CRM_HORARIO_LABEL) || isUsableHorarioEvento(extracted.horario_evento);
...
export function isFieldSatisfied(
  ...
  switch (field) {
    ...
    case "fecha":
      return (
        filledSet.has(CRM_FECHA_LABEL) || isUsableFechaEvento(extracted.fecha_evento)
      );
    case "horario":
      return (
        filledSet.has(CRM_HORARIO_LABEL) || isUsableHorarioEvento(extracted.horario_evento)
      );
    case "presupuesto":
...
const MINIMAL_SERVICE_PATTERN =
  /\b((?:solo|so)\s+)?(mesas?\s+y\s+sillas?|sillas?\s+y\s+mesas?|renta\s+de\s+(mesas?|sillas?)|(?:solo|so)\s+(mesas?|sillas?|mobiliario))\b/i;
...
export function looksLikeMinimalServiceAsk(text: string | null | undefined): boolean {
  ...
  // A15917: "solo/so mobiliario" sin pieza → menú de pieza, no soft complement de mesas/sillas.
  if (
    /\b(?:solo|so)\s+mobiliario\b/i.test(text) &&
    !/\b(mesas?\s+y\s+sillas?|sillas?\s+y\s+mesas?|mesas?|sillas?|periqueras?)\b/i.test(
      text.replace(/\b(?:solo|so)\s+mobiliario\b/gi, " ")
    )
  ) {
    return false;
  }
  return MINIMAL_SERVICE_PATTERN.test(text);
}
...
export function applyLucyMessageGuards(input: LucyMessageGuardsInput): string {
  ...
  if (cierreYaEnviado && clientAsksPhone(currentMessage)) {
    ...
    allowSalesReplyOverride &&
    !cierreYaEnviado &&
    /\b(?:solo|so)\s+mobiliario\b|^mobiliario$/i.test((currentMessage ?? "").trim()) &&
    !/\b(mesas?|sillas?|periqueras?|lounge)\b/i.test(currentMessage ?? "") &&
    !shouldSkipSalesMenuForConcreteQuestion(currentMessage)
  ) {
    filledSet.add("Requerimientos o servicios");
    extracted.requerimientos_evento =
      mergeServiceRequirements(extracted.requerimientos_evento, "Mobiliario", 6) ||
      "Mobiliario";
    mensaje = mergeWithPendingQuestion(
    ...
    appliedSalesReply = true;
    appliedDirectReply = true;
    log?.info({ entityId }, "GUARD: A15917 — solo mobiliario → pieza + ambos catálogos");
  } else if (
    ...
    const pending = getNextPendingField(extracted, filledSet);
    const nextQ = pending ? buildNaturalQuestion(pending, ctx) : null;
    const fechaHint = extracted.fecha_evento?.trim()
      ? ` Para el *${extracted.fecha_evento.trim()}* confirmamos disponibilidad en cuanto tengamos el resto de datos.`
    ...
    if (piece === "entelados" || piece === "entelado" || piece === "colgantes" || piece === "colgante") {
      ...
      // Pieza concreta: un catálogo; bare mobiliario ya trae ambos links en el body.
      const withLink =
        piece === "mobiliario"
          ? body
          : (() => {
              const catalogUrl =
                (piece === "periqueras" || piece === "salas lounge"
                  ? getCatalogWebUrlForQuery("periqueras")
                  : getCatalogWebUrlForQuery("mesas y sillas")) ||
                getCatalogWebHubDeliveryUrl();
              return catalogUrl && !/bodasesor\.com\/catalogos/i.test(body)
                ? `${body}\n\nCatálogo:\n${catalogUrl}`
                : body;
            })();
      mensaje = mergeWithPendingQuestion(
  ...
  if (fechaFromMsg && isUsableFechaEvento(fechaFromMsg) && !filledSet.has(CRM_FECHA_LABEL)) {
    ...
    syncLegacyFechaHorarioField(extracted);
    if (
      mensajeAsksForField(mensaje, "horario") ||
      /quieres que te d[eé] detalles|siguiente dato del evento/i.test(mensaje)
    ) {
      const nextQ = nextFieldQuestion(
        extracted,
  ...
  if (
    ...
    const variant = nombre
      ? `${pickTransition(presHistory)} ${nombre}, ¿tienen ya el día del evento?`
      : `${pickTransition(presHistory)} ¿Tienen ya el día del evento?`;
  ...
  // Si el cliente pidió mobiliario sin pieza, forzar menú + ambos catálogos.
  {
    ...
    const looksLikeHubOnly =
      ...
      (/Te dejo el catálogo general/i.test(mensaje) &&
        !/mesas-y-sillas|salas-y-periqueras/i.test(mensaje));
    if (
      !cierreYaEnviado &&
      (bareMobAsk || crmBareMob) &&
      (looksLikeHubOnly ||
        (!/mesas-y-sillas/i.test(mensaje) &&
          !/salas-y-periqueras/i.test(mensaje) &&
          /bodasesor\.com\/catalogos\/?(?:\s|\?|$)/i.test(mensaje)))
    ) {
      const offer = buildBareMobiliarioOfferBlock();
      mensaje = mergeWithPendingQuestion(offer, filledSet, extracted, ctx);
      log?.info({ entityId }, "GUARD: A15917 — mobiliario bare → pieza + ambos catálogos");
    }
  }
