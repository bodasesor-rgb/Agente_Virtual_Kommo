...
function isOwnCompanyEmail(email) {
  const norm3 = normalizeEmail(email);
  if (!norm3) return false;
  if (OWN_EMAILS.has(norm3)) return true;
  return /@bodasesor\.com$/i.test(norm3) || /@capybaraeventos\./i.test(norm3);
}
function filterClientEmail(email) {
  const norm3 = normalizeEmail(email);
  if (!norm3 || isOwnCompanyEmail(norm3)) return null;
  return email.trim();
}
...
function looksLikeValidClientEmail(email) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(norm3)) return false;
  const domain = norm3.split("@")[1] ?? "";
...
function clearSheetSynonymIndex() {
  sheetSynonymIndex = /* @__PURE__ */ new Map();
}
...
function defaultFamiliesAsPdfAliases() {
...
var PDF_ALIAS_FAMILIES = defaultFamiliesAsPdfAliases();
...
function aliasesForPdfLabel(fileName, serviceLabel) {
  const hay = norm2(`${fileName} ${serviceLabel}`);
  const out = new Set(synonymsForServiceName(serviceLabel).map(norm2));
  for (const t of hay.split(" ").filter((w) => w.length >= 3)) out.add(t);
  for (const fam of PDF_ALIAS_FAMILIES) {
    const matchesHint = fam.pdfHints.some((h) => hay.includes(norm2(h)));
...
var snapshot = null;
function serviceLabelFromPdfName(name) {
  return name.replace(/\.pdf$/i, "").replace(/[_-]+/g, " ").replace(/\s+/g, " ").replace(/\bBodasesor\b/gi, "").replace(/\b2026\b/g, "").replace(/\bbodaseor\b/gi, "").trim();
}
...
function buildDrivePdfServiceAnswer(query) {
  ...
  return `S\xED manejamos *${label}*. Del cat\xE1logo 2026:
...
function clearDrivePdfSnapshotForTests() {
  snapshot = null;
}
  ...
  if (pdfBlock || hasDrivePdfKnowledge(trimmed)) {
    return {
      ...
      promptBlock: [
        ...
        `Servicio: ${label}`,
        "Hay descripci\xF3n/men\xFA en el cat\xE1logo PDF. \xDAsalo para explicar qu\xE9 incluye.",
        "NUNCA cites precios del PDF. Precio: 'nuestro equipo te confirma seg\xFAn invitados/nivel'.",
...
function getCatalogStatus() {
  return snapshot2?.status ?? emptyStatus();
}
function setCatalogSnapshotForTests(rows) {
  ...
  snapshot2 = { rows, promptBlock: "", status };
...
function catalogKeywordsFromQuery(query) {
  ...
  if (expanded.matchedServiceHints.length) {
    ...
    const primary = source.slice(0, 2).flatMap((hint) => normalizeForMatch(hint).split(/\s+/).filter((x) => x.length >= 4));
...
function catalogAnswerMatchesRequestedService(query, answer) {
  const keywords = catalogKeywordsFromQuery(query);
  if (!keywords.length) return true;
  const norm3 = normalizeForMatch(answer);
  return keywords.every((k) => norm3.includes(k));
}
...
function resolveCatalogQuery(query) {
  if (!snapshot2?.rows.length) return null;
  const allRows = snapshot2.rows;
  const trimmed = query.trim();
  if (!trimmed) return null;
...
function lookupCatalogServices(query) {
  if (!snapshot2?.rows.length) return [];
  return rankCatalogMatches(query, snapshot2.rows, false);
}
...
function summarizeServicePrices(serviceKey, maxLevels = 4) {
  const rows = snapshot2?.rows.filter(
...
function buildCatalogCateringOverviewFromSheet() {
  ...
  for (const row of snapshot2.rows) {
...
function bodyEqualsLastAssistant(msg, history, clientName) {
  ...
  const norm3 = (s) => stripLeadingTransition(s).trim();
  const a = norm3(msg);
  const b = norm3(last.content);
  if (a === b) return true;
...
var ChatCompletionStream = class _ChatCompletionStream extends AbstractChatCompletionRunner {
  ...
  [(_ChatCompletionStream_params = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_choiceEventStates = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_currentChatCompletionSnapshot = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_instances = /* @__PURE__ */ new WeakSet(), _ChatCompletionStream_beginRequest = function _ChatCompletionStream_beginRequest2() {
    ...
    const snapshot3 = __classPrivateFieldGet(this, _ChatCompletionStream_currentChatCompletionSnapshot, "f");
    if (!snapshot3) {
      throw new OpenAIError(`request ended without sending any chunks`);
    }
    __classPrivateFieldSet(this, _ChatCompletionStream_currentChatCompletionSnapshot, void 0, "f");
    __classPrivateFieldSet(this, _ChatCompletionStream_choiceEventStates, [], "f");
    return finalizeChatCompletion(snapshot3, __classPrivateFieldGet(this, _ChatCompletionStream_params, "f"));
    ...
    var _a3, _b, _c, _d;
    let snapshot3 = __classPrivateFieldGet(this, _ChatCompletionStream_currentChatCompletionSnapshot, "f");
    const { choices, ...rest } = chunk;
    if (!snapshot3) {
      snapshot3 = __classPrivateFieldSet(this, _ChatCompletionStream_currentChatCompletionSnapshot, {
        ...rest,
        choices: []
      }, "f");
    } else {
      Object.assign(snapshot3, rest);
    }
    for (const { delta, finish_reason, index, logprobs = null, ...other } of chunk.choices) {
      let choice = snapshot3.choices[index];
      if (!choice) {
        choice = snapshot3.choices[index] = { finish_reason, index, message: {}, logprobs, ...other };
      }
      if (logprobs) {
        if (!choice.logprobs) {
...
function finalizeChatCompletion(snapshot3, params) {
  const { id, choices, created, model, system_fingerprint, ...rest } = snapshot3;
  const completion = {
    ...rest,
    id,
...
_a = AssistantStream, _AssistantStream_addEvent = function _AssistantStream_addEvent2(event) {
      ...
      if (event.data.delta.content) {
        for (const content of event.data.delta.content) {
          if (content.type == "text" && content.text) {
            ...
            let snapshot3 = accumulatedMessage.content[content.index];
            if (snapshot3 && snapshot3.type == "text") {
              this._emit("textDelta", textDelta, snapshot3.text);
            } else {
  ...
  switch (event.event) {
    case "thread.run.step.created":
      ...
      return event.data;
    case "thread.run.step.delta":
      let snapshot3 = __classPrivateFieldGet(this, _AssistantStream_runStepSnapshots, "f")[event.data.id];
      if (!snapshot3) {
        throw Error("Received a RunStepDelta before creation of a snapshot");
      }
      let data = event.data;
      if (data.delta) {
        const accumulated = _a.accumulateDelta(snapshot3, data.delta);
...
}, _AssistantStream_accumulateMessage = function _AssistantStream_accumulateMessage2(event, snapshot3) {
  let newContent = [];
  switch (event.event) {
    case "thread.message.created":
    ...
    case "thread.message.delta":
      ...
      if (data.delta.content) {
        for (const contentElement of data.delta.content) {
          if (contentElement.index in snapshot3.content) {
            let currentContent = snapshot3.content[contentElement.index];
            snapshot3.content[contentElement.index] = __classPrivateFieldGet(this, _AssistantStream_instances, "m", _AssistantStream_accumulateContent).call(this, contentElement, currentContent);
          } else {
            snapshot3.content[contentElement.index] = contentElement;
            newContent.push(contentElement);
          }
        }
      }
      return [snapshot3, newContent];
    case "thread.message.in_progress":
    case "thread.message.completed":
    case "thread.message.incomplete":
      if (snapshot3) {
...
function accumulateResponse(event, snapshot3) {
  if (!snapshot3) {
  ...
  switch (event.type) {
    case "response.output_item.added": {
      snapshot3.output.push(structuredClone(event.item));
      if (event.item.type === "message") {
        addOutputText(snapshot3);
      }
      break;
    }
    case "response.output_item.done": {
      getOutput(snapshot3, event.output_index);
      snapshot3.output[event.output_index] = structuredClone(event.item);
      if (event.item.type === "message") {
        addOutputText(snapshot3);
      }
      break;
    ...
    case "response.content_part.added": {
      const output = getOutput(snapshot3, event.output_index);
    ...
    case "response.output_text.delta": {
      ...
      if (output.type === "message") {
        ...
        snapshot3.output_text += event.delta;
      }
      break;
    }
    ...
    case "response.incomplete": {
      snapshot3 = cloneResponse(event.response);
      break;
    }
...
function cloneResponse(response) {
  const snapshot3 = structuredClone(response);
  if (!Object.getOwnPropertyDescriptor(snapshot3, "output_text") || snapshot3.output_text == null) {
    addOutputText(snapshot3);
  }
  return snapshot3;
}
function getOutput(snapshot3, outputIndex) {
  const output = snapshot3.output[outputIndex];
  if (!output) {
...
var ResponseStream = class _ResponseStream extends EventStream {
  ...
  [(_ResponseStream_params = /* @__PURE__ */ new WeakMap(), _ResponseStream_currentResponseSnapshot = /* @__PURE__ */ new WeakMap(), _ResponseStream_finalResponse = /* @__PURE__ */ new WeakMap(), _ResponseStream_instances = /* @__PURE__ */ new WeakSet(), _ResponseStream_beginRequest = function _ResponseStream_beginRequest2() {
    ...
    const snapshot3 = __classPrivateFieldGet(this, _ResponseStream_currentResponseSnapshot, "f");
    if (!snapshot3) {
      throw new OpenAIError(`request ended without sending any events`);
    }
    __classPrivateFieldSet(this, _ResponseStream_currentResponseSnapshot, void 0, "f");
    const parsedResponse = finalizeResponse(snapshot3, __classPrivateFieldGet(this, _ResponseStream_params, "f"));
...
function finalizeResponse(snapshot3, params) {
  return maybeParseResponse(snapshot3, params);
}
...
async function runAll() {
  ...
  await test("10. Integraciones \u2014 m\xF3dulos conectados y features activas", () => {
    ...
    const norm3 = normalizeAdvisorReferences(
      "Le paso estos datos a Alejandro para que te arme una cotizaci\xF3n.",
      "Alejandro"
    );
    assert.ok(norm3.includes("nuestro equipo"));
  ...
  await test("18. Ver\xF3nica A14760 \u2014 por aqu\xED sin correo, sin Alejandro, nombre completo", () => {
    ...
    assert.ok(norm3.includes("nuestro equipo"));
    assert.ok(!/Alejandro/i.test(norm3));
  });
  ...
  await test("31. A14786 \u2014 cliente Alejandro: saludo correcto, no confundir con asesor", () => {
    ...
    assert.ok(/Mucho gusto,\s+Alejandro/i.test(norm3), norm3);
    assert.ok(/nuestro equipo te arme/i.test(norm3), norm3);
  ...
  await test("46. Drive PDF RAG \u2014 fichas aprendidas y conocimiento enriquecido", () => {
    ...
    const synServed = expandQueryWithServiceSynonyms("quiero men\xFA formal emplatado");
  ...
  await test("47. Sheet columna Sin\xF3nimos + matcher enriquecido", () => {
    clearSheetSynonymIndex();
    const csv = [
      '"Servicio","Nivel","Precio Unitario","Precio Minimo de salida","Cat\xE1logo Revisado","Que Incluye","Sinonimos"',
      '"Taquiza","Solo Alimentos","$300.00","$9,000.00","TRUE","5 guisados","tacos, taquiza para evento, puesto de tacos"',
      '"Banquete Formal","Basico","$500.00","$15,000.00","TRUE","3 tiempos","men\xFA formal, comida servida, cena formal"',
    ...
    setCatalogSnapshotForTests(rows);
    const fromSheet = expandQueryWithServiceSynonyms("quiero men\xFA formal");
    assert.ok(
      fromSheet.familyKeys.includes("banquete_formal") || fromSheet.matchedServiceHints.some((h) => /banquete|formal/i.test(h)),
    ...
    assert.ok(tacosPrice, "tacos debe resolver a Taquiza v\xEDa sin\xF3nimos");
